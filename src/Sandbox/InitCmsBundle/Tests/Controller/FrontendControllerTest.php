...
class FrontendControllerTest extends WebTestCase
{
        ...
        public function testLanguageSwitchToEnglish()
    {
        ...
        $crawler = $client->followRedirect();
        $this->assertEquals(200, $client->getResponse()->getStatusCode(), 'no further redirect');
        $this->assertTrue($crawler->filter('.hero-unit p:contains("The locale of this page is en_US")')->count() == 1, 'Language is switched');
        $this->assertTrue($crawler->filter('.header ul li.active a:contains("English")')->count() == 1, 'active is English');
        $this->assertTrue($crawler->filter('.header ul li.active a:contains("Deutsch")')->count() == 0, 'german is not active');
    }
